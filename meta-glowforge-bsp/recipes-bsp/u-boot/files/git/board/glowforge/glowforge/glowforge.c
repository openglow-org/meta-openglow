...
static void read_board_rev(void)
{
  ...
  if (board_rev == 0) {
    u32 serial_number = readl(&fuse->mac_addr0);
...
static void enter_recovery_mode(void)
{
  ...
  if (((boot_fuses & EMMC_BOOT_FUSES) == EMMC_BOOT_FUSES) &&
      (blk_get_device_part_str("mmc", EMMC_RECOVERY_PARTITION_STR, NULL, NULL, 0) == 1)) {
    printf("*** Recovery partition detected\n");
    env_set("mmcroot", EMMC_MMCROOT);
    env_set("mmcdev", __stringify(EMMC_MMCDEV));
    env_set("mmchwpart", __stringify(EMMC_MMCHWPART));
    env_set("mmcpart", __stringify(EMMC_MMCPART));
    env_set("boot_recovery", "yes");
  } else {
    hang_in_uboot();
  }
...
{
  if (env_get_yesno("env_saved") == -1) {
    env_set("env_saved", "yes");
    env_save();
  }
...
#ifdef PSTORE_SIZE
  env_set_hex("pstore_addr", gd->ram_top);
  env_set_hex("pstore_size", PSTORE_SIZE);
#endif

  env_set("board_rev", board_rev_str);
  env_set("board_rev_major", board_rev_major_str);
  env_set("boot_fuses", boot_fuses_str);
#ifdef CONFIG_IMX_WATCHDOG
  hw_watchdog_init();
#endif
